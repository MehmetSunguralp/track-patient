...
export default function MetabolicPowerScreen() {
  ...
  const latestPower = samples.at(-1)?.movement.metabolicPowerWkg || 0;
  
  const chartData = useMemo(() => 
    samples.slice(-6).map((item) => ({
      value: Math.round(item.movement.metabolicPowerWkg * 10) / 10,
      label: item.timestamp.slice(11, 19),
    })),
    [dataLength, latestTimestamp, latestPower, selectedPatient.id]
  );
      ...
      {chartData.length > 0 ? (
        ...
        <View style={styles.noDataContainer}>
          <Text style={styles.noDataText}>
            No metabolic power data available for this patient. Select a connected patient from the
            status bar.
          </Text>
        </View>
      )}
