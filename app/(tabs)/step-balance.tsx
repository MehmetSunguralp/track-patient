...
export default function StepBalanceScreen() {
  ...
  const latestBalance = samples.at(-1)?.movement.stepBalancePercent || 0;
  
  const chartData = useMemo(() => 
    samples.slice(-6).map((item) => ({
      value: Math.round(item.movement.stepBalancePercent * 10) / 10,
      label: item.timestamp.slice(11, 19),
    })),
    [dataLength, latestTimestamp, latestBalance, selectedPatient.id]
  );
      ...
      {chartData.length > 0 ? (
        ...
        <View style={styles.noDataContainer}>
          <Text style={styles.noDataText}>
            No step balance data available for this patient. Select a connected patient from the
            status bar.
          </Text>
        </View>
      )}
