...
export default function DistanceScreen() {
  ...
  const latestDistance = samples.at(-1)?.gps.distanceTotalM || 0;
  
  const chartData = useMemo(() => 
    samples.slice(-6).map((item) => ({
      value: item.gps.distanceTotalM,
      label: item.timestamp.slice(11, 19),
    })),
    [dataLength, latestTimestamp, latestDistance, selectedPatient.id]
  );
      ...
      {chartData.length > 0 ? (
        ...
        <View style={styles.noDataContainer}>
          <Text style={styles.noDataText}>
            No distance data available for this patient. Select a connected patient from the
            status bar.
          </Text>
        </View>
      )}
